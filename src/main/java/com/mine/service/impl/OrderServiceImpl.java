...
import com.mine.vo.CartOrderItemVO;
import com.mine.vo.OrderItemVO;
import com.mine.vo.OrderVO;
import com.mine.vo.ShippingVO;
...
public class OrderServiceImpl implements OrderService {
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId) {
        ...
        if (shippingId == null) {
            return ServerResponse.serverResponseError("地址参数不能为空");
        }
        ...
        if (order == null) {
            return ServerResponse.serverResponseError("订单创建失败");
        }
        ...
        return ServerResponse.serverResponseSuccess(orderVO);
    }
    ...
    public ServerResponse cancel(Integer userId, Long orderNo) {
        ...
        if (order == null) {
            return ServerResponse.serverResponseError("订单不存在");
        }
        //step3:判断订单状态并取消
        if (order.getStatus() != Const.OrderStatusEnum.ORDER_UN_PAY.getCode()) {
            return ServerResponse.serverResponseError("订单不可取消");
        }
        //step4:返回结果
        ...
        return ServerResponse.serverResponseError("订单取消失败");
    }
    ...
    public ServerResponse get_order_cart_product(Integer userId) {
        ...
        cartOrderItemVO.setImageHost(PropertiesUtils.getValue("imageHost"));
        List<OrderItem> orderItemList = (List<OrderItem>) serverResponse.getData();
        List<OrderItemVO> orderItemVOList = Lists.newArrayList();
        if (orderItemList == null || orderItemList.size() == 0) {
            return ServerResponse.serverResponseError("购物车空");
        }
        for (OrderItem orderItem : orderItemList) {
        ...
        //step4:返回结果
        return ServerResponse.serverResponseSuccess(cartOrderItemVO);
    }
    ...
    public ServerResponse list(Integer userId, Integer pageNum, Integer pageSize) {
        ...
        if (orderList == null || orderList.size() == 0) {
            return ServerResponse.serverResponseError("未查询到订单信息");
    ...
    private ServerResponse getCartOrderItem(Integer userId, List<Cart> cartList) {
        ...
        for (Cart cart : cartList) {
            ...
            if (product == null) {
                return ServerResponse.serverResponseError("id为" + cart.getProductId() + "的商品不存在");
            }
            if (product.getStatus() != Const.ProductCodeEnum.PRODUCT_ONLINE.getCode()) {//商品下架
                return ServerResponse.serverResponseError("id为" + product.getId() + "的商品已经下架");
            }
            if (product.getStock() < cart.getQuantity()) {//库存不足
                return ServerResponse.serverResponseError("id为" + product.getId() + "的商品库存不足");
            }
        ...
        return ServerResponse.serverResponseSuccess(orderItemList);
    }
    ...
    public ServerResponse pay(Integer userId, Long orderNo) {
        ...
        if (orderNo == null) {
            return ServerResponse.serverResponseError("订单号不能为空");
        }
        ...
        return ServerResponse.serverResponseError("下单失败");
    }
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
        ...
        Order order = orderMapper.findOrderByOrderNo(orderNo);
        if (order == null) {
            return ServerResponse.serverResponseError("订单" + orderNo + "不是本商品的订单");
        }

        if (order.getStatus() >= Const.OrderStatusEnum.ORDER_PAYED.getCode()) {
            //防止支付宝重复回调
            return ServerResponse.serverResponseError("支付宝重复调用");
        }
        ...
        if (trade_status.equals(Const.TRADE_SUCCESS)) {
            ...
            order.setPaymentTime(DateUtils.stringToDated(payment_time));
        ...
        return ServerResponse.serverResponseError("支付信息保存失败");
    }
    ...
    public ServerResponse query_order_pay_status(Long orderNo) {
        ...
        if (order.getStatus() == Const.OrderStatusEnum.ORDER_PAYED.getCode()) {
            return ServerResponse.serverResponseSuccess(true);
        }
        return ServerResponse.serverResponseSuccess(false);
    }
